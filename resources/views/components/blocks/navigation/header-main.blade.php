...
<nav x-data="{ mobileMenuOpen: false }"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            {{-- Logo --}}
            <a href="{{ LaravelLocalization::localizeUrl($home_url) }}" class="flex items-center space-x-3 group">
                @if($logo)
                    ...
                    @foreach($items as $item)
                        @if(($item['type'] ?? 'link') === 'link')
                            <a href="{{ LaravelLocalization::localizeUrl($item['url']) }}"
                               class="text-slate-700 dark:text-slate-300 hover:text-red-600 dark:hover:text-red-400 transition-all duration-200 flex items-center space-x-2 font-medium group">
    ...
    <div x-show="mobileMenuOpen"
        ...
        <div class="px-4 py-3 space-y-2">
            @foreach($items as $item)
                @if(($item['type'] ?? 'link') === 'link')
                    <a href="{{ LaravelLocalization::localizeUrl($item['url']) }}"
                       class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-red-50 dark:hover:bg-red-950/20 hover:text-red-600 dark:hover:text-red-400 transition-colors">
